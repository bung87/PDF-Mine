...
class PDFMine:
    ...
    def save_video(self, targetdir):
        ...
        for page in PDFPage.create_pages(self.doc):
            if (page.annots):
                obj = self.doc.getobj(page.annots)
                for i in obj:
                    annotobj = i.resolve()
                    try:
    ...
    def _intersects(self, layout, obj):
        ...
        for otherobj in layout:
            if obj != otherobj:
                ...
                if (origbbox[0] >= otherbbox[0]) and (origbbox[1] >= otherbbox[1]) and (
                    origbbox[2] <= otherbbox[2]) and (origbbox[3] >= otherbbox[3]):
                    return otherbbox
        return origbbox
